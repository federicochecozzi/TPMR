...
ThreeMonthTrendOrderQty(ProductID,[Year],[Month],MonthNumber,OrderQty,Trend) AS (
	SELECT ProductID,[Year],[Month],MonthNumber,OrderQty,(3*sumxy-sumx*sumy)/(3*sumxx-sumx*sumx)
	FROM SumOrderQtyTable
)
SELECT *
FROM ThreeMonthTrendOrderQty
...
WHERE t.ProductID = dp.ProductKey AND MonthNumber = 3
ORDER BY Trend DESC