...
SumOrderQtyTable(Category,[Year],[Month],MonthNumber,OrderQty,sumx,sumy,sumxx,sumxy) AS (
	SELECT Category,[Year],[Month],MonthNumber,OrderQty, 
		   SUM(MonthNumber) OVER(PARTITION BY Category ORDER BY MonthNumber ROWS BETWEEN 2 PRECEDING AND CURRENT ROW),
		   SUM(OrderQty) OVER(PARTITION BY Category ORDER BY MonthNumber ROWS BETWEEN 2 PRECEDING AND CURRENT ROW),
		   SUM(MonthNumber*MonthNumber) OVER(PARTITION BY Category ORDER BY MonthNumber ROWS BETWEEN 2 PRECEDING AND CURRENT ROW),
		   SUM(OrderQty*MonthNumber) OVER(PARTITION BY Category ORDER BY MonthNumber ROWS BETWEEN 2 PRECEDING AND CURRENT ROW)
	FROM ExtendedOrderQtyTable
),
ThreeMonthTrendOrderQty(Category,[Year],[Month],MonthNumber,OrderQty,Trend) AS (
	SELECT Category,[Year],[Month],MonthNumber,OrderQty,(3*sumxy-sumx*sumy)/(3.0*sumxx-sumx*sumx)
	FROM SumOrderQtyTable
)
SELECT t.Category, dpc.EnglishProductCategoryName , t.Trend
FROM ThreeMonthTrendOrderQty t, AdventureWorksDW2019.dbo.DimProductCategory dpc 
WHERE t.Category = dpc.ProductCategoryKey AND MonthNumber = 38
ORDER BY Trend DESC